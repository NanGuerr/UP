# 📝 Transcripción y Resolución Detallada de Examen (Álgebra Lineal y Matemática Discreta) 🧮

Este documento contiene la transcripción completa de las preguntas de las evaluaciones mostradas en las imágenes, junto con su respectiva resolución detallada paso a paso, aplicando el formato LaTeX corregido (sin comandos erróneos como `rac`, usando delimitadores correctos y evitando cualquier etiqueta `cite`).
...
3. Calculamos el vector normal $\n$ del plano mediante el producto vectorial $\vec{u} \times \vec{v}$:
...
5. Aplicamos la fórmula de distancia de un punto $P(x_0, y_0, z_0)$ al plano $Ax + By + Cz + D = 0$:
   ...
   $$d = \frac{|8(1) + 9(2) + 23(3) - 65|}{\sqrt{8^2 + 9^2 + (-23)^2}} = \frac{|8 + 18 + 69 - 65|}{\sqrt{64 + 81 + 529}} = \frac{|30|}{\sqrt{674}} = \frac{30}{\sqrt{674}}$$
...
3. **Vector unitario con la misma dirección de $\vec{p} + 3\vec{q}$:**
   ...
   Norma: $\|(1, 1, 12)\| = \sqrt{1^2 + 1^2 + 12^2} = \sqrt{1 + 1 + 144} = \sqrt{146}$.
