# 📘 Autoevaluación Ecu. Dif. Exactas y Factor Integrante
...
2. 🧮 **Calculamos las derivadas parciales cruzadas:**
   - Derivada parcial de $M$ respecto a $y$ (tratamos a $x$ como constante): $\frac{\partial M}{\partial y} = e^{k^{2}x} \cdot 4\cos(4y) = 4e^{k^{2}x}\cos(4y)$
   - Derivada parcial de $N$ respecto a $x$ (tratamos a $y$ como constante): $\frac{\partial N}{\partial x} = k^{2}e^{k^{2}x}\cos(4y)$
...
3. ⚖️ **Igualamos ambas expresiones** ($\frac{\partial M}{\partial y} = \frac{\partial N}{\partial x}$):
   ...
   $$4 = k^2 \implies k = \pm \sqrt{4} \implies k = 2 \text{ o } k = -2$$
