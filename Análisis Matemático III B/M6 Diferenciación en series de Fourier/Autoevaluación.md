...
* **Análisis Teórico:** 
  La función $f(x) = x^{1/3}$ es continua en todo el intervalo $[-l, l]$. Su derivada es $f'(x) = rac{1}{3}x^{-2/3} = rac{1}{3\sqrt{x^2}}$ para $x 
eq 0$. 
...
* **Análisis Teórico:** 
  ...
  $$rac{d}{dx}[SF2] = 6 \cdot SF1$$
  Multiplicando la serie derivada por la constante $rac{1}{6}$, se obtiene exactamente $SF1$.
* **Opción Correcta:** **B. Derivando término a término $SF2$ y multiplicando por $rac{1}{6}$, se obtiene $SF1$.**
...
* **Análisis Teórico:** 
  ...
  $$f(x) = rac{a_0}{2} + \sum_{n=1}^{\infty} a_n \cos\left(rac{n\pi}{l}x
ight)$$
  Al integrar término a término entre dos límites $c$ y $d$, la integral de cada término en coseno es $\int \cos\left(rac{n\pi}{l}x
ight)dx = rac{l}{n\pi} \sin\left(rac{n\pi}{l}x
ight)$. De este modo, los sumandos de la serie numérica resultante son únicamente términos en seno (evaluados en los límites).
...
* **Análisis Teórico:** 
  ...
  $$f(x) = \sum_{n=1}^{\infty} b_n \sin\left(rac{n\pi}{3}x
ight)$$
  Al integrar término a término entre 0 y 2, la integral de $\sin\left(rac{n\pi}{3}x
ight)$ resulta en $rac{3}{n\pi}\left[1 - \cos\left(rac{2n\pi}{3}
ight)
ight]$ evaluada en los límites. Así, la serie numérica resultante posee únicamente términos en coseno.
* **Opción Correcta:** **Verdadero.**



### ❓ Pregunta 6
* **Consigna:** Sea $f(x)$ con derivada suave en todo $(-l,l)$, dada por su serie de Fourier $rac{a_0}{2} + \sum_{n=1}^{\infty} \left[a_n \cos\left(rac{n\pi}{l}x
ight) + b_n \sin\left(rac{n\pi}{l}x
ight)
ight]$. Al aplicar el teorema para derivación a $f(x)$ y luego el teorema para integración entre 0 y $x$:
* **Análisis Teórico:**
  * Al derivar término a término $f(x)$, se elimina la constante $rac{a_0}{2}$ y se obtiene la serie de $f'(x)$.
...
* **Opción Correcta:** **B. $f(x) - f(0) = \sum_{n=1}^{\infty} \left[ a_n \cos\left(rac{n\pi}{l}x
ight) + b_n \sin\left(rac{n\pi}{l}x
ight) 
ight]$ con convergencia puntual.**
...
* **Análisis Teórico:** 
  La serie de Fourier de una función impar solo contiene términos en seno: $f(x) = \sum_{n=1}^{\infty} b_n \sin\left(rac{n\pi}{l}x
ight)$. Al derivar término a término, la derivada de $\sin\left(rac{n\pi}{l}x
ight)$ es $\left(rac{n\pi}{l}
ight)\cos\left(rac{n\pi}{l}x
ight)$. Puesto que la derivada de una función impar es una función par, la serie derivada resultante está compuesta únicamente por términos en coseno.
