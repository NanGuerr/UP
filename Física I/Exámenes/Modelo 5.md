# ⚛️ Parcial de Física 1 - Ejercicios de Dinámica y Movimiento 



## 🎢 1. Energía Mecánica, Resortes y Fuerzas Disipativas
...
Aplicamos el principio de conservación de la energía mecánica entre el punto inicial (en reposo a altura $h$) y el punto de máxima compresión del resorte:
$$E_{	ext{mec, inicial}} = E_{	ext{mec, final}}$$
$$m \cdot g \cdot h = \frac{1}{2} k \cdot \Delta x^2$$

Sustituyendo los valores ($m = 5\text{ kg}$, $h = 10\text{ m}$, $g = 9.8\text{ m/s}^2$ o $10\text{ m/s}^2$, $k = 18000\text{ N/m}$):
...
La fuerza elástica máxima ocurre en el punto de máxima compresión del resorte:
$$F_{	ext{max}} = k \cdot \Delta x = 18000 \text{ N/m} \times 0.236\text{ m} \approx 424.26\text{ N}$$

#### c) Descripción del movimiento del bloque
1. **Descenso:** El bloque acelera al descender por la rampa curva bajo la acción de la gravedad, convirtiendo su energía potencial gravitatoria en energía cinética.
2. **Impacto y compresión:** Al llegar a la planicie horizontal sin rozamiento, se desplaza con velocidad constante hasta colisionar con el resorte, donde desacelera armónicamente hasta detenerse por completo en la compresión máxima.
3. **Rebote:** El resorte se descomprime, restituyendo la energía almacenada al bloque y propulsándolo de regreso en sentido inverso.

#### d) Tramo con rozamiento de $0.8\text{ m}$ ($\mu = 0.2$)
El trabajo realizado por la fuerza de rozamiento en el tramo horizontal disipa energía mecánica:
$$W_{	ext{roz}} = -F_{	ext{roz}} \cdot d = -(\mu \cdot m \cdot g) \cdot d$$
$$W_{	ext{roz}} = -(0.2 \cdot 5 \cdot 10) \cdot 0.8 = -10 \cdot 0.8 = -8\text{ J}$$

La energía mecánica remanente para comprimir el resorte es:
...
Calculamos la energía disipada en este tramo largo:
$$W_{	ext{roz}} = -(\mu \cdot m \cdot g) \cdot d = -(0.95 \cdot 5 \cdot 10) \cdot 100 = -47.5 \cdot 100 = -4750\text{ J}$$

> 💡 **Interpretación profunda:** El trabajo disipativo de la fuerza de rozamiento ($-4750\text{ J}$) es diez veces mayor que la energía potencial gravitatoria inicial disponible ($500\text{ J}$). Esto significa que **el bloque jamás llega a alcanzar el resorte**; se detiene por completo a los $500 / 47.5 \approx 10.53\text{ m}$ de haber ingresado a la planicie horizontal.



## 📦 2. Ecuaciones de Newton (Selección Múltiple)
...
* **Opciones del sistema:**
  * $\Sigma F_y = 0 \implies N - P = 0$
  * $\Sigma F_x = 0 \implies F - F_{	ext{roz, din}} = 0$ (o bien $F = F_{	ext{roz, din}}$ dado que al moverse a velocidad constante la aceleración es $a = 0$).
...
#### a) Cantidad de movimiento vectorial ($ ec{p} = m \cdot  ec{v}$)
Definimos un sistema de coordenadas cartesiano: Este ($+x$), Oeste ($-x$), Norte ($+y$), Sur ($-y$).
...
#### b) Rapidez y dirección posterior al choque inelástico
* Masa total combinada: $M = m_1 + m_2 = 1300 + 3000 = 4300\text{ kg}$
* Módulo del momento lineal total:
  $$|\vec{p}_{	ext{total}}| = \sqrt{(-2700)^2 + 3900^2} = \sqrt{7290000 + 15210000} = \sqrt{22500000} \approx 4743.42 \text{ kg}\cdot\text{m/s}$$
* Velocidad final conjunta ($v_f$):
  $$v_f = \frac{|\vec{p}_{	ext{total}}|}{M} = \frac{4743.42}{4300} \approx 1.10\text{ m/s}$$
* Dirección del movimiento:
  $$\theta = \arctan\left(\frac{p_y}{p_x}\right) = \arctan\left(\frac{3900}{-2700}\right) \approx -55.3^\circ \text{ (hacia el cuadrante Noroeste)}$$



## 🐴 4. Dinámica y Cinemática del Frenado
...
La velocidad final es $v_f = 0\text{ m/s}$ en un intervalo $t = 5\text{ s}$.
...
$$\Delta x = v_0 \cdot t + \frac{1}{2} a \cdot t^2 = 15(5) + \frac{1}{2}(-3)(5)^2 = 75 - 37.5 = 37.5\text{ m}$$

#### b) Módulo de la fuerza horizontal aplicada
...
#### a) Velocidad al retornar al piso
Por la simetría del tiro parabólico sin resistencia del aire, la componente horizontal permanece constante y la componente vertical invierte su sentido ($v_y = -6\text{ m/s}$). El módulo de la velocidad final al impactar el suelo es idéntico al inicial:
$$v_{	ext{final}} = \sqrt{v_{0x}^2 + v_y^2} = \sqrt{8^2 + (-6)^2} = \sqrt{100} = 10\text{ m/s}$$

#### b) Tiempo hasta la altura máxima
En el punto más alto, la componente vertical de la velocidad se anula ($v_y = 0$):
