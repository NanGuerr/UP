# 📡 Plan de Estudios 2026 🎓

## 📐 Semestre 3 (Foco en Matemáticas y Física) — Agosto - Diciembre 2026

1. 📚 `(026318)` Análisis Matemático IIIa (Matemática Avanzada)
2. ⚛️ `(026095)` Física I (Físicas IIa y IIb)
3. 💻 `(026178)` Sistemas Digitales II (Lab. de Microprocesadores)
4. 📐 `(023209)` Álgebra Lineal (Robótica)
5. 📈 `(025054)` Análisis Matemático IIIb (Señales y Matemática Avanzada)
6. 📊 `(025915)` Probabilidad y Estadística (Procesos Estocásticos)

## ☀️ Verano — Enero - Febrero 2027

* 📻 `(021436)` Conatel Taller de Radio

## 🚀 Semestre 4 (Destrabando la rama de Señales) — Marzo - Julio 2027

1. 📐 `(026177)` Sistemas de Representación para Electrónica
2. ⚡ `(024823)` Física IIa
3. 🖥️ `(025612)` Arquitectura de Computadores (Sistemas Operativos y Embebidos)
4. 〰️ `(026438)` Señales y Sistemas (Matemático IIIb)
5. 🧮 `(026435)` Matemática Avanzada (Electromagnetismo)
6. 🧲 `(026204)` Física IIb (Dispositivos Electrónicos y Lab. de Electrónica)

## 🔌 Semestre 5 (La base de la Electrónica) — Agosto - Diciembre 2027

1. 💡 `(026439)` Dispositivos Electrónicos (Física IIb)
2. 🧲 `(026446)` Electromagnetismo (Matemática Avanzada)
3. ⚙️ `(026440)` Teoría de Circuitos (Señales y Sistemas + Física IIb)
4. 🎲 `(026448)` Procesos Estocásticos (Señales y Sistemas + Probabilidad)
5. 📈 `(024886)` Teoría de la Información (Probabilidad y Estadística)
6. 🔬 `(023959)` Laboratorio de Electrónica (Física IIb) ❔

## ☀️ Verano — Enero - Febrero 2028

* ☁️ `(027221)` DevOps (Electiva)

## 📡 Semestre 6 (Comunicaciones e Infraestructura) — Marzo - Julio 2028

1. 🔌 `(026442)` Circuitos Electrónicos (Dispositivos Electrónicos + Teoría de Circuitos)
2. 📞 `(026450)` Sistemas de Comunicaciones Digitales y Analógicos (Procesos Estocásticos)
3. 💰 `(026532)` Fundamentos de Economía y Finanzas 🌞
4. 📶 `(026447)` Procesamiento de Señales (Procesos Estocásticos + Álgebra Lineal) 
5. 📻 `(023968)` Conducción e Irradiación de Ondas (Electromagnetismo) 🥼
6. 🤖 `(027129)` Introducción a los Sistemas Embebidos (Arquitectura del Computador)

## 🌐 Semestre 7 (Sistemas Complejos) — Agosto - Diciembre 2028

1. 💻 `(026437)` Laboratorio de Microprocesadores (Sistemas Digitales II y Lab. Electrónica) 🥼
2. 🛰️ `(023962)` Dispositivos de Comunicaciones (Electromagnetismo y Circuitos Electrónicos)
3. ⚡ `(023961)` Circuitos Electrónicos II (Circuitos Electrónicos) 🥼
4. 👔 `(026009)` Organización, Gestión y Entrepreneurship (Fundamentos de Economía y Autorización del Director de Departamento)
5. 🦾 `(027178)` Introducción a la Robótica (Electiva Algebra Lineal y Física I) 🌞
6. 📡 `(026684)` Comunicaciones Ópticas e Inalámbricas (Conducción e Irradiación de Ondas y Sistemas de Comunicaciones Digitales y Analógicos)

## ☀️ Verano — Enero - Febrero 2029
🔄 (027130) Sensores y Acond. de Señal (Circuitos Electrónicos)

> * *01/01/16 13, 27/12/19 9, 12/02/21 8*

## 🛠️ Semestre 8 (Redes y Gestión) — Marzo - Julio 2029

1. 🖧 `(026214)`  Redes de Backbone (Comunicaciones ópticas e inalámbricas y Circuitos electrónicos)
2. 🌐 `(026535)` Redes de Acceso (Comunicaciones ópticas e inalámbricas y Circuitos electrónicos)
3. 🔬 `(023965)` Laboratorio de Comunicaciones (Lab. Electrónica y Dispositivos de Comunicaciones) 🥼
4. 🖥️ `(025418)` Sistemas Operativos (Electiva Arquitectura de Computadores) 🌞
5. 📡 `(024829)` Circuitos para Radiofrecuencias (Dispositivos de Comunicaciones) 🥼
6. 📋 `(027515)` Formulación de Proyectos y del Trabajo Final (Fundamentos de Economía y Finanzas)

## 🎓 Semestre 9 (Final de Carrera) — Agosto - Diciembre 2029

1. 🏢 `(027510)` Práctica Profesional Supervisada (Autorización del Director de Departamento)
2. ⚖️ `(025701)` Ingeniería Legal
3. 📜 `(027513)` Trabajo Final de Grado (Formulación de Proyectos y autorización del Director de Departamento)
4. 📊 `(025920)` Modelos y Simulación (Electiva Probabilidad y Estadística)
5. 🌐 `(026213)` TCP/IP
6. 🛡️ `(026319)` Seguridad Ambiental y del Trabajo


> 💡 **Nota de Cursada:** Recuerda verificar los requerimientos de presencialidad obligatoria en la sede CABA de la Universidad de Palermo para materias específicas como Laboratorio de Electrónica, Circuitos Electrónicos II, Conducción y Radiación de Ondas, Circuitos de Radiofrecuencia y Laboratorio de Comunicaciones.
