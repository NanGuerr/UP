...
4. **¿Por qué se creó un paquete de componentes (`componentes_pkg.vhd`)?**
   * *Respuesta:* Para organizar de manera modular todas las declaraciones de entidades, permitiendo que cualquier módulo de nivel superior (como `TP_CuentaPPS.vhd` o los testbenches) pueda instanciarlos simplemente invocando `use work.componentes.all;`.
